// ===== FX TAE TRADING JOURNAL - PROFESSIONAL EDITION =====
// Complete updated script with all requested features
...
function updateAccountBalanceDisplay() {
    ...
    if (totalDepositsDisplayEl) totalDepositsDisplayEl.textContent = formatCurrency(totalDeposits);
    
    const totalWithdrawalsDisplayEl = document.getElementById('totalWithdrawalsDisplay');
    if (totalWithdrawalsDisplayEl) totalWithdrawalsDisplayEl.textContent = formatCurrency(totalWithdrawals);
    ...
    if (equityTotalEl) equityTotalEl.textContent = formatCurrency(accountBalance);
}
...
function processDeposit() {
    ...
    if (startingBalance === 0 && deposits.length === 0) {
...
function updateAllTradesTable() {
    ...
    document.getElementById('buysSellsRatio').textContent = `${buys}:${sells}`;
}
        ...
        options: {
                ...
                x: {
                    ...
                    ticks: {
                        ...
                        callback: function(val, index) {
                            ...
                            // Show date only for current day, dots for others
            ...
            onHover: (event, chartElement) => {
                ...
                if (chartElement.length) {
                    ...
                    const dataset = equityChart.data.datasets[0];
                    const label = equityChart.data.labels[dataIndex];
                    const value = dataset.data[dataIndex];
                    
                    document.getElementById('tooltipDate').textContent = label;
                    document.getElementById('tooltipValue').textContent = formatCurrency(value);
...
function getEquityData(period) {
    ...
    deposits.forEach(d => {
        const date = d.date;
        if (!dailyChanges.has(date)) dailyChanges.set(date, 0);
        dailyChanges.set(date, dailyChanges.get(date) + d.amount);
    });
    
    withdrawals.forEach(w => {
        const date = w.date;
        if (!dailyChanges.has(date)) dailyChanges.set(date, 0);
        dailyChanges.set(date, dailyChanges.get(date) - w.amount);
    });
    
    trades.forEach(t => {
        const date = t.date;
        if (!dailyChanges.has(date)) dailyChanges.set(date, 0);
        dailyChanges.set(date, dailyChanges.get(date) + t.pnl);
    });
    ...
    if (period === '1m') {
        ...
        // If no trades in period, show last month with starting balance
        ...
        filteredDates = sortedDates.filter(date => new Date(date) >= twelveMonthsAgo);
        
        // For 12m, we want monthly aggregates
        const monthlyData = new Map();
        
        filteredDates.forEach(date => {
            const monthKey = date.substring(0, 7); // YYYY-MM
            if (!monthlyData.has(monthKey)) monthlyData.set(monthKey, 0);
            monthlyData.set(monthKey, monthlyData.get(monthKey) + dailyChanges.get(date));
        });
...
function updateEquityStats(data) {
    ...
    document.getElementById('equityDrawdown').textContent = `${drawdown}%`;
}
...
function generateStyledPDF({ title, content, filename, color = '#3b82f6' }) {
    try {
        ...
        pdf.setFontSize(12);
        pdf.setFont('helvetica', 'bold');
        pdf.text(`Trader: ${user.name || 'Trader'}`, margin, 50);
        ...
        lines.forEach(line => {
            ...
            if (line.startsWith('TRADE DETAILS') || line.startsWith('ACCOUNT SUMMARY') || 
                ...
                pdf.text(line, margin, yPos);
                yPos += 5;
            } else if (line.includes('LOSS') || line.includes('-')) {
...
function exportAnalyticsPDF() {
    ...
    const content = `
        ...
        Largest Win: ${formatCurrency(Math.max(...winningTrades.map(t => t.pnl), 0))}
        Largest Loss: ${formatCurrency(Math.min(...losingTrades.map(t => t.pnl), 0))}
...
function exportTransactionsPDF() {
    const content = `
        ...
        ===============
        ${deposits.map(d => `${d.date} ${d.time} | ${d.broker} | +${formatCurrency(d.amount)} | Balance: ${formatCurrency(d.balanceAfter)}`).join('\n')}
        
        WITHDRAWAL HISTORY (${withdrawals.length})
        ==================
        ${withdrawals.map(w => `${w.date} ${w.time} | ${w.broker} | -${formatCurrency(w.amount)} | Balance: ${formatCurrency(w.balanceAfter)}`).join('\n')}
    `;
...
function exportAllDataPDF() {
    ...
    const content = `
        ...
        ============
        ${trades.map(t => `${t.date} ${t.time} | #${t.tradeNumber} | ${t.pair} | ${t.direction} | ${t.strategy} | ${formatCurrencyWithSign(t.pnl)}`).join('\n')}
...
function setupEventListeners() {
    ...
    if (depositAmount) {
        depositAmount.addEventListener('input', function() {
            ...
            document.getElementById('newBalanceAfterDeposit').value = (accountBalance + amount).toFixed(2);
        });
    }
...
function initializeAuthPage() {
    // Auth page initialization - to be added if needed
}
...
(function() {
    console.log('🚀 Running emergency loader fix...');
    
    const forceShowPage = function() {
        const loader = document.getElementById('loader');
        const mainContainer = document.getElementById('mainContainer');
        ...
        if (typeof Chart === 'undefined') {
            console.warn('⚠️ Chart.js not found - charts will not display');
            window.Chart = function() {
                console.log('Chart.js placeholder');
            };
        }
    };
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', forceShowPage);
    }
    
    window.addEventListener('load', function() {
        setTimeout(forceShowPage, 100);
    });
    
    [10, 50, 100, 300, 500, 1000, 2000].forEach(timeout => {
        setTimeout(forceShowPage, timeout);
    });
